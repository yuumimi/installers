			...
			(
				for i in {1..8}; do
					if grep -q "sing-box started" "$singbox_log_file"; then
						singbox_start_message
						sleep 3
						$open_url "https://get.geojs.io/" && $open_url "https://youtube.com" && $open_url "$YACD"
						break
					fi
					sleep 1
				done
		...
		windows)
			...
			(
				if reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\chrome.exe" >/dev/null 2>&1; then
					...
					sleep 3
					start chrome.exe "$YACD" "https://youtube.com" "https://get.geojs.io/" --dns-prefetch-disable --proxy-pac-url="$PAC"
				else
				...
				if reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\msedge.exe" >/dev/null 2>&1; then
					...
					sleep 3
					start msedge.exe "$YACD" "https://youtube.com" "https://get.geojs.io/" --dns-prefetch-disable --proxy-pac-url="$PAC"
				fi
			) &

		...
		download_config() {
			...
			if [ -f "${WEBI_PKG_WORKDIR}/config.tmp" ]; then
				if "$pkg_dst_cmd" check -c "${WEBI_PKG_WORKDIR}/config.tmp" 2>&1; then
					...
					windows)
						if [ -z "${default_interface:-}"]; then
							default_interface=$(ipconfig | awk '/Ethernet adapter/ {gsub(/:/,"",$3); print $3}')
						fi
						;;
					esac

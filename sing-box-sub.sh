...
__bootstrap_webi() {
	...
	WEBI_RELEASES='https://github.com/yangchuansheng/sing-box/releases/download'
	WEBI_PKG='sing-box'
	PKG_NAME='sing-box'
	WEBI_OS="${OS}"
	WEBI_ARCH="${ARCH}"
	...
	WEBI_PKG_FILE="${PKG_NAME}-${WEBI_OS}-${WEBI_ARCH}${WEBI_EXT}"
	WEBI_PKG_URL="${WEBI_HOST}/${WEBI_RELEASES}/${WEBI_VERSION}/${WEBI_PKG_FILE}"
	WEBI_UA="$(uname -a)"
	WEBI_PKG_DOWNLOAD=""
	...
	# 	echo ""
	# 	printf "Thanks using '\e[32m%s\e[0m' on '\e[31m%s/%s\e[0m'.\n" "${WEBI_PKG:-}" "$(uname -s)" "$(uname -m)"
	# 	echo ""
		...
		_sudo() {
			...
			windows)
				if [[ $(sfc 2>&1 | tr -d '\0') =~ SCANNOW ]]; then
					...
					printf "\n权限不足,必须以管理员身份运行 Git Bash:\n右键点击桌面上的 Git Bash 图标 > 属性 > 兼容性 > 勾选以管理员身份运行此程序 > 确定\n\n"
					"${@}"
				fi
				;;
			esac
